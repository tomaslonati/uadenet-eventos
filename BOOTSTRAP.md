# BOOTSTRAP.md — inicialización del repo `uadenet-eventos`

Este documento es para que Claude Code (u otro agente) inicialice el monorepo desde cero, sin re-derivar decisiones que ya están tomadas. Antes de tocar código, leer `AGENTS.md` (convenciones) y todo `docs/` (contexto y arquitectura) — este archivo asume que ya se leyeron.
...
- Node LTS activa instalada (ver `docs/01-arquitectura.md` por la versión vigente).
...
2. `packages/eslint-config` — reglas compartidas. (Si se decide usar Biome en vez de ESLint/Prettier, actualizar este paso y `docs/01-arquitectura.md` antes de seguir — no está cerrado, ver `03-backlog.md`.)
...
4. `packages/contracts` — carpeta `src/` vacía por ahora salvo un `index.ts` placeholder; se llena a medida que se cierre `docs/02-modelo-dominio.md` (ver ADR 0005: Zod, no generado desde OpenAPI).
5. `packages/db`:
   ...
   - `src/schema/` — vacío por ahora, esperar a que el modelo de dominio esté validado (`docs/02-modelo-dominio.md` todavía tiene TBDs).
...
4. Crear la estructura de carpetas vacía con placeholders: `src/modules/`, `src/workers/`, `src/common/guards/`, `src/common/interceptors/`, `src/common/helpers/` (ver `docs/01-arquitectura.md` para el árbol completo).
...
7. **No crear todavía los módulos `eventos/`, `inscripciones/`, `asistencia/`** — dependen de que el modelo de dominio y el backlog estén cerrados (`docs/02-modelo-dominio.md`, `docs/03-backlog.md`). Si en el momento de correr esto ya están cerrados, sí generarlos siguiendo la estructura feature-based documentada.
...
1. Completar `README.md` de la raíz con: qué es el proyecto, cómo instalar (`pnpm install`), cómo correr en dev (`turbo run dev`), cómo correr las migrations cuando existan, link a `docs/00-overview.md` para contexto completo.
...
3. Actualizar `docs/03-backlog.md`: marcar la tarea técnica "Setup del monorepo" como hecha, y volcar cualquier decisión tomada durante el bootstrap que no estuviera ya en `docs/decisions/` (agregar el ADR correspondiente si hizo falta decidir algo nuevo en el camino).

---

## Qué NO hacer en este bootstrap

- No modelar el schema de Drizzle todavía (depende de cerrar `docs/02-modelo-dominio.md`).
- No implementar el guard de validación de JWT de CORE con una lógica inventada — dejarlo como placeholder documentado (ver `docs/04-integraciones.md`, sigue pendiente).
